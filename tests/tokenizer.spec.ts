...
describe('preTokenize 分段', () => {
  ...
  it('中文连段与标点分开，R2/R3', () => {
    ...
    // 苹果 1 段、空格负号、前置空格、你好 1 段，与 Rust 逐字节一致
  ...
  it('空格+负号合成一段，R3 分支 3，与 Rust 一致', () => {
    ...
    // 合并产物为 Ġ- 一个 token，id 565 等于 256 加 rank 309
...
describe('bpeMerge', () => {
  it('hello 合并为单 token，id 与官方 transformers 一致', () => {
    ...
    // 官方标准 V4-Flash tokenizer 对 hello 输出单 token [33310]
    expect(ids).toEqual([33310])
  })

  it('存在中文多字 token：习近平新时代中国特色社会主义思想', () => {
...
describe('splitWithAdded 分段', () => {
  ...
  it('非特殊 added token 即 response 命中后仍按 bpe 段处理', () => {
...
describe('IncrementalTokenizer 增量一致性', () => {
  ...
  it('真实长句重复 100 次：增量、一次性、官方三者精确一致，覆盖截断路径', () => {
    // 真实混合中英长句，含空格和标点，pre-token 拆成多段，有真实词级合并，重复 100 次
