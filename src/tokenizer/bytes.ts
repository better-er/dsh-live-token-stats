/**
 * ByteLevel 字节↔unicode 映射（GPT-2 风格，与 HuggingFace tokenizers 的
 * ByteLevel pre-tokenizer 逐字节一致）。
 *
 * 语义：0x21-0x7E、0xA1-0xAC、0xAE-0xFF 映射到自身，其余字节按序映射到
 * 0x100 起的 unicode 码位（如空格→`Ġ`）。
 *
...
/** 把 UTF-8 字节序列映射为 unicode 字符串（BPE 操作的载体，1 字节 = 1 码位）。 */
