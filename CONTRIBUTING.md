...
3. 提 PR 到 `main`，按正文自述改动与自测情况；描述建议覆盖：改动内容、自测勾选、行为变化、测试说明。
4. 触发 `ci.yml`：它会在 `main` 的 push 与所有 PR 上自动跑 typecheck + test + build。
...
- 不要在 PR 中提交 `lib/` 之外的多余产物，`node_modules`、`*.tsbuildinfo` 均由 `.gitignore` 排除。
