...
- **偏差如实对账**：纯文本与推理的输出偏差约 ±1%；工具调用因官方额外计费消息模板与调用 id，本地如实在结算后显示偏差而非伪装精确。
...
- 工具调用会话的估算会低于官方实际值：官方对 tool-call 消息的模板包装与调用 id 等额外计费，本地 delta 拿不到这部分，偏差随工具形态变化并在结算后如实显示。纯文本与推理的输出偏差在 ±1% 内。
